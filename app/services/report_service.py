...
async def get_daily_report(
    ...
    """Get a daily report by date, returning None if missing."""
    result = await db.execute(
        select(DailyReport).where(DailyReport.report_date == report_date)
    )
...
async def get_or_create_daily_report(
    ...
    """Alias for get_or_create_report."""
    return await get_or_create_report(db, report_date)
...
async def save_morning_plan(
    ...
    """Save morning plan and update tasks_planned count."""
    report = await get_or_create_report(db, report_date)
    report.morning_plan = plan
    if tasks_planned:
        report.tasks_planned = tasks_planned
...
async def save_evening_summary(
    ...
    """Save evening summary with insights, metrics snapshot, and task counts."""
    report = await get_or_create_report(db, report_date)
    report.evening_summary = summary
