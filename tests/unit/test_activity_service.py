...
from app.services.activity_service import get_recent_activities, log_activity
...
async def test_log_activity_publishes_to_redis(async_db_session, mock_redis):
    ...
    assert "activity" in call_args[0]
...
async def test_get_recent_activity_returns_ordered(async_db_session, mock_redis):
    ...
    results = await get_recent_activities(async_db_session, limit=10)
    assert len(results) == 2
    # Last inserted has higher ID
